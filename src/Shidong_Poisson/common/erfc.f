...
c
        if(x .lt. 1.0) goto 1400
        call errfunc(x,erfunc)
        erfun=1-erfunc
...
c
     ...
     5     c02(38),c24(30),c46(30),c68(30),c810(30)
        equivalence 
...
c        c at the point x
c
cccc        call prin2('in errexev, x=*',x,1)
cccc        call prin2('in errexev, c=*',c,10)
        x2=x+x
        tkm1=1
        tk=x
        fun=c(1)*tkm1
...
c
c
        subroutine errfmap(xin,a,xout,xmin,xmax,xmid,wt)
        implicit real *8 (a-h,o-z)
c
c	this subroutine maps a point xin\in [-a,3*a] to 
c	a point xout \in [xmin,xmax] through the errfunc.
c	More specifically, the map is constructed in the following
c	way:
c	   [-a,a] ==> [xmin,xmid] via
c	        xout=(xmid-xmin)/2*erf(xin)/erf(a)+(xmid+xmin)/2
c	   [a,3*a] ==> [xmid,xmax] via
c		xout=(xmax-xmid)/2*erf(xin-2*a)/erf(a)+(xmax+xmid)/2
c
	done=1
	pi=4*atan(done)
	coef=2/sqrt(pi)
...
c
	if (xin.le.a) then
	  call errfun(xin,xerr)
	  xout=(xmid-xmin)/2/aerr
	  wt=xout*coef*exp(-xin*xin)
	  xout=xout*xerr+(xmid+xmin)/2
	  return
	endif
c
c	for xin \in [a,3*a]:
c
	if (xin.gt.a) then
          xar=xin-2*a
	  call errfun(xar,xerr)
	  xout=(xmax-xmid)/2/aerr
	  wt=xout*coef*exp(-xar*xar)
	  xout=xout*xerr+(xmax+xmid)/2
	  return
	endif
c  
        return
...
c
c
        subroutine errfmap0(ier,xin,a,xout,xmin,xmax,wt)
        implicit real *8 (a-h,o-z)
c
c	this subroutine maps a point xin\in [-a,a] to 
c	a point xout \in [xmin,xmax] through the errfunc.
c	More specifically, the map is constructed in the following
c	way:
c	   [-a,a] ==> [xmin,xmid] via
...
c	for xin \in [-a,a]:
c
	if ((xin.lt.-a).or.(xin.gt.a)) then
	  ier=16
	  return
	endif
c
	call errfun(xin,xerr)
	xout=(xmax-xmin)/2/aerr
	wt=xout*coef*exp(-xin*xin)
	xout=xout*xerr+(xmax+xmin)/2
c  
        return
        end
c
